...
public class MyInfoService : IMyInfoService
{
    ...
    public async Task<MyInfoAuthorizationRequest> CreateAuthorizationRequestAsync(string state)
    {
        EnsureConfigured();
        ...
        string dpopPrivateKey)
    {
        ...
        var tokenExchange = await ExchangeCodeForTokensAsync(discovery, authCode, codeVerifier, dpopKey);
        ...
        bool validateLifetime)
    {
        ...
        var validationParameters = new TokenValidationParameters
        {
            ...
            IssuerSigningKeys = await GetIssuerSigningKeysAsync(discovery.JwksUri),
    ...
    private async Task<IReadOnlyCollection<SecurityKey>> GetIssuerSigningKeysAsync(string jwksUri)
    {
        ...
        var responseBody = await _httpClient.GetStringAsync(jwksUri);
        using var document = JsonDocument.Parse(responseBody);
    ...
    private async Task<MyInfoDiscoveryDocument> GetDiscoveryDocumentAsync()
    {
        ...
        using var response = await _httpClient.GetAsync(_discoveryUrl);
        var responseBody = await response.Content.ReadAsStringAsync();
        if (!response.IsSuccessStatusCode)
        {
            _logger.LogError(
