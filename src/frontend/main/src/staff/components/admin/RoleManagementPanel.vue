...
const showAccessFunctionModal = ref(false);
const accessFunctionForm = ref<SaveAccessFunctionRequest>({
  code: "",
  name: "",
  description: null,
  module: "Admissions",
...
const accessFunctionModules = computed(() => {
  const modules = new Set(
    staffAccessFunctions.value.map((item) => item.module),
  );
...
const accessFunctionCodePreview = computed(() => {
  const explicitCode = accessFunctionForm.value.code.trim();
  return explicitCode || buildCode(accessFunctionForm.value.name);
});
...
function resetAccessFunctionForm(): void {
  accessFunctionForm.value = {
    code: "",
    name: "",
    description: null,
    module: accessFunctionModules.value[0] ?? "Admissions",
    isActive: true,
    displayOrder: 0,
  };
}

function openCreateAccessFunction(): void {
  resetAccessFunctionForm();
  showAccessFunctionModal.value = true;
}
...
async function saveAccessFunction(): Promise<void> {
  const accessFunctionName = accessFunctionForm.value.name.trim();
  const accessFunctionCode = accessFunctionCodePreview.value;
  const accessFunctionModule = accessFunctionForm.value.module.trim();

  if (!accessFunctionName) {
    toast.error("Access function name is required");
    return;
  }

  if (!accessFunctionCode) {
    toast.error("Access function code is required");
    return;
  }

  if (!accessFunctionModule) {
  ...
  try {
    await roleService.saveAccessFunction({
      id: accessFunctionForm.value.id,
      code: accessFunctionCode,
      name: accessFunctionName,
      description: accessFunctionForm.value.description?.trim() || null,
      module: accessFunctionModule,
      isActive: accessFunctionForm.value.isActive,
      displayOrder: accessFunctionForm.value.displayOrder,
    });

    emit("refresh");
    toast.success("Access function added");
    showAccessFunctionModal.value = false;
  } catch (error: unknown) {
    toast.error(getApiErrorMessage(error, "Failed to save access function"));
  } finally {
    saving.value = false;
  }
}
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        ...
        <p class="mt-1 text-sm text-slate-500">
          Combine admissions and enrolment access functions in one role, then
          assign that role to any staff member.
        </p>
      </div>
      <div class="flex flex-wrap gap-3">
        <button
          ...
          @click="openCreateAccessFunction"
        >
    ...
    >
      <div class="mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <p class="mt-1 text-sm text-slate-500">
            These are the permissions available when building shared staff
            roles.
          </p>
        </div>
        <span
        ...
        >
          ...
          >
            <div>
              ...
              <p class="mt-1 text-sm text-slate-500">
                Build one shared role with admissions and enrolment access
                functions.
              </p>
            </div>
            <button
          ...
          >
            <section class="flex flex-col gap-5">
              ...
              <div class="rounded-2xl border border-accent/10 bg-accent/5 p-5">
                ...
                <p class="mt-1 text-sm text-slate-500">
                  Roles can combine permissions from admissions, enrolment, and
