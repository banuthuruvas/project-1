...
function getSessionId(): string {
  const key = import.meta.env.VITE_COOKIE_SESSION_KEY;
  return Cookie.get(key) ?? "";
}

function getUserId(): string {
  const key = import.meta.env.VITE_COOKIE_USER_KEY;
...
export function useSignalR() {
  async function start() {
    ...
    const hubUrl = `${baseUrl}/hubs/notifications?userId=${encodeURIComponent(userId)}&roles=${encodeURIComponent(roles)}`;
